...
SIDE_MAP = {
    'BUY': 'BUY', 'B': 'BUY', 'BOT': 'BUY',
    'COVER': 'BUY', 'BUYTOCOVER': 'BUY', 'C': 'BUY',
    'SELL': 'SELL', 'S': 'SELL', 'SLD': 'SELL',
    'SHORT': 'SELL', 'SS': 'SELL', 'SELLSHORT': 'SELL',
}
...
<style>
    ...
    :root {
        ...
        --accent:#00ff88; --accent2:#ff3366; --text:#e8e8f0; --muted:#555570; --card:#13131c;
    }
...
def compute_fifo(all_trades):
    ...
    for t in all_trades:
        ...
        rows.append({
            'Symbol':      t.get('symbol'),
            'Side':        side,
            'Antal':       float(t.get('qty') or 0),
            'Pris':        float(t.get('price') or 0),
    ...
    for symbol, group in df.groupby('Symbol'):
        ...
        for _, row in group.iterrows():
            qty   = abs(float(row['Antal']))
            price = abs(float(row['Pris']))
            side  = row['Side']
            ts    = row['Tidsstämpel']
            datum = row['Datum']
            ...
            # Track entry time when opening from flat
            ...
            if side == 'BUY':
                if position < -0.01:
                    ...
                    duration_min = round((ts - entry_time).total_seconds() / 60) if entry_time else None
                    valid_trades.append({'Ticker': symbol, 'Vinst ($)': round(pnl,2),
                        'Vinst %': round(pct,1), 'Riktning': 'SHORT',
                        'Datum': datum, 'Tidsstämpel': ts, 'Hålltid (min)': duration_min})
                    if remaining > 0.01: long_fifo.append((remaining, price))
                    if abs(position + qty) < 0.01: entry_time = None
                else:
                    if abs(position) < 0.01: entry_time = ts
                ...
                if position > 0.01:
                    ...
                    duration_min = round((ts - entry_time).total_seconds() / 60) if entry_time else None
                    valid_trades.append({'Ticker': symbol, 'Vinst ($)': round(pnl,2),
                        'Vinst %': round(pct,1), 'Riktning': 'LONG',
                        'Datum': datum, 'Tidsstämpel': ts, 'Hålltid (min)': duration_min})
...
st.markdown("""<div class="app-header">
    <span class="app-title">TRADING JOURNAL</span>
    <span class="app-subtitle">// TRADEZERO ACCOUNT ANALYTICS</span>
...
with st.sidebar:
    ...
    st.markdown(f"""<div style="font-family:Space Mono,monospace;font-size:0.65rem;color:#555570;line-height:1.8;">
        KONTO: {ACCOUNT_ID or 'EJ KONFIGURERAT'}<br>
        UPPDATERAD: {datetime.now().strftime('%H:%M:%S')}</div>""", unsafe_allow_html=True)
...
if not all_trades:
    st.error("Inga trades. Kontrollera API-nycklar i .env-filen."); st.stop()

res_df = compute_fifo(all_trades)

if not res_df.empty:
    res_df['Datum_dt'] = pd.to_datetime(res_df['Datum'], errors='coerce')
    mask = ((res_df['Datum_dt'] >= pd.Timestamp(date_from)) &
            (res_df['Datum_dt'] <= pd.Timestamp(date_to)))
    ...
    filtered = res_df[mask & dir_mask].copy()
else:
    filtered = pd.DataFrame()


# --- LIVE P&L ---
# Correct field names from API: dayRealized, totalUnrealized, dayPnl
...
# Sum commissions from individual positions
commissions = sum(float(p.get('dayRealizedPnl', 0) or 0) for p in pnl_data.get('pnl', []))
...
p1, p2 = st.columns(2)
with p1:
    long_tickers = " | ".join(p['symbol'] for p in longs)
    st.markdown(mcard("LONG", len(longs), "int", sub=long_tickers or "–"), unsafe_allow_html=True)
with p2:
    short_tickers = " | ".join(p['symbol'] for p in shorts)
    st.markdown(mcard("SHORT", len(shorts), "int", sub=short_tickers or "–"), unsafe_allow_html=True)
...
if not filtered.empty:
    ...
    cols = st.columns(8)
    stats = [
        ("NETTO P/L",       total_pnl,       "dollar"),
        ("WIN RATE",        win_rate,        "pct"),
        ("TRADES",          total,           "int"),
        ("AVG VINST $",     avg_win_dollar,  "dollar"),
        ("AVG FÖRLUST $",   avg_loss_dollar, "dollar"),
        ("AVG VINST %",     avg_win_pct,     "pct"),
        ("AVG FÖRLUST %",   avg_loss_pct,    "pct"),
        ("RISK/REWARD",     rr,              "x"),
    ]
    for col, (label, val, fmt) in zip(cols, stats):
    ...
    with t1: st.markdown(mcard("VINNANDE TRADES", avg_win_time or 0,  "time", sub="genomsnitt"), unsafe_allow_html=True)
    with t2: st.markdown(mcard("FÖRLORANDE TRADES", avg_loss_time or 0, "time", sub="genomsnitt"), unsafe_allow_html=True)
    all_time = filtered['Hålltid (min)'].dropna().mean()
    with t3: st.markdown(mcard("ALLA TRADES", all_time or 0, "time", sub="genomsnitt"), unsafe_allow_html=True)
    ...
    if not filtered.empty:
        ...
        with col_a:
            ...
            fig.add_trace(go.Scatter(x=daily['Datum'], y=daily['Kumulativ'],
                ...
                line=dict(color='#00ff88', width=2), name='Kumulativ P/L',
        ...
        with col_b:
            ...
            fig2 = go.Figure(go.Bar(x=daily['Datum'], y=daily['Vinst ($)'],
                marker_color=colors, hovertemplate='%{x}<br>$%{y:,.2f}<extra></extra>'))
...
with tab2:
    if not filtered.empty:
        ...
        # Treemap heatmap: size = antal trades, color = avg vinst %
        by_ticker['AbsTrades'] = by_ticker['Trades']  # size must be positive
        by_ticker['Label'] = by_ticker.apply(
            lambda r: f"{r['Ticker']}<br>{r['AvgPct']:+.1f}%<br>${r['PnL']:+,.0f}", axis=1)

        fig3 = px.treemap(
            by_ticker, path=['Label'], values='AbsTrades', color='AvgPct',
            color_continuous_scale=[
                [0.0, '#ff3366'],   # red for worst
                [0.4, '#ff6633'],   # orange
                [0.5, '#444444'],   # neutral
                [0.6, '#33aa66'],   # light green
                [1.0, '#00ff88'],   # bright green for best
            ],
            color_continuous_midpoint=0,
        )
        fig3.update_layout(
            **PLOTLY_LAYOUT, height=450, title='P/L per Ticker (storlek = antal trades)',
            coloraxis_colorbar=dict(title='Avg %', tickfont=dict(color='#888899')),
        )
        fig3.update_traces(
            textfont=dict(family='Space Mono', size=11),
            hovertemplate='%{label}<br>Trades: %{value}<extra></extra>',
            marker=dict(line=dict(width=1, color='#1e1e2e')),
        )
        st.plotly_chart(fig3, width='stretch')

        st.dataframe(by_ticker[['Ticker','PnL','Trades','Win%','AvgPct']]
            .rename(columns={'PnL':'P/L ($)','AvgPct':'Avg %'}),
...
with tab3:
    if not filtered.empty:
        ...
        by_month = monthly.groupby('Månad').agg(
            ...
            AvgHålltidVinst=('Hålltid (min)', lambda x: x[monthly.loc[x.index,'Vinst ($)'] > 0].mean()),
            AvgHålltidFörlust=('Hålltid (min)', lambda x: x[monthly.loc[x.index,'Vinst ($)'] < 0].mean()),
        ).reset_index()
        ...
        by_month['Hålltid Vinst']  = by_month['AvgHålltidVinst'].apply(fmt_duration)
        by_month['Hålltid Förlust']= by_month['AvgHålltidFörlust'].apply(fmt_duration)

        # Monthly P/L bar chart — categorical x-axis to align bars with labels
        colors = ['#00ff88' if v > 0 else '#ff3366' for v in by_month['PnL']]
        fig4 = go.Figure(go.Bar(x=by_month['Månad'], y=by_month['PnL'],
        ...
        fig4.update_layout(**PLOTLY_LAYOUT, height=280, title='Månadsvis P/L',
                           xaxis_type='category')
        st.plotly_chart(fig4, width='stretch')

        st.dataframe(
            by_month[['Månad','Trades','Vinster','Förluster','Win%','PnL',
                       'AvgVinstPct','AvgFörlustPct','Hålltid Vinst','Hålltid Förlust']]
            .rename(columns={'PnL':'P/L ($)','AvgVinstPct':'Avg Vinst %',
                             'AvgFörlustPct':'Avg Förlust %'}),
            width='stretch', hide_index=True)
    else:
        st.info("Inga trades i valt intervall.")
...
with tab4:
    if not filtered.empty:
        display = filtered[['Datum','Ticker','Riktning','Vinst ($)','Vinst %','Hålltid (min)']].copy()
        ...
        def color_pnl(val):
            if isinstance(val, (int, float)):
                return f'color: {"#00ff88" if val > 0 else "#ff3366" if val < 0 else "#888899"}; font-family: Space Mono, monospace;'
            return ''
...
with tab5:
    raw_df = pd.DataFrame([{
        ...
        'Qty':     t.get('qty'),
        'Pris':    t.get('price'),
        'Netto':   t.get('netProceeds'),
        'TradeID': t.get('tradeId'),
    } for t in all_trades])

    ticker_filter = st.selectbox("Filtrera ticker",
        options=['Alla'] + sorted(raw_df['Symbol'].dropna().unique().tolist()))
    if ticker_filter != 'Alla':
        raw_df = raw_df[raw_df['Symbol'] == ticker_filter]

    st.dataframe(raw_df.sort_values(['Datum','Tid'], ascending=False),
